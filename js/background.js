...
var playing = false;
if (!lsGet("badgeStatus")) { lsSet("badgeStatus", "off") };
...
// For tab numbers
var msgTooManyTabs = ["OUCH! Too Many Tabs", "You opened more than " + lsGet("maxTabs") + " tabs! Close them down and keep your sanity!", "Yeap! Keep on closing them!"];
...
var msgBlacklisted = ["BLACKLISTED SITE!!!", "Watch out! You have " + lsGet("timeWindow") + " seconds before the zap! Click here to close this page!", "blacklisted", ""];
var msgZaped = ["Ouch!", "Too much time on blacklisted sites! Hurry outta here! Another zap is coming in " + lsGet("timeWindow") + " secs!", ""];


var accessToken = lsGet("accessToken");
...
function createTimeout() {
	fireRescueTime(lsGet("RTAPIKey"));
	var x = setTimeout(function () {
		// Clearing up
		RTTimeOut = false;
		lsSet("RTTimeOut", false);
	}, parseInt(lsGet("RTFrequency")) * 60 * 1000); // Gotta put this on 3 minutes
	lsSet("RTTimeOut", x);
	return x
}
...
function fireRescueTime(APIKey) {
	...
	$.get(requestAddress)
		.done(function (data) {
			lsSet("RTPulse", data.pulse);
			lsSet("RTHour", data.comment.split(" ")[9]);
			lsSet("Comment", data.comment);

			log("Productivity pulse from 30 minutes before " + lsGet("RTHour") + "is " + data.pulse)

			var prod = data.pulse;
			if (prod == 0 || prod == null || prod == 'null') { return }
			else if (prod < parseInt(lsGet("RTNegLimit"))) {
				notifyUser("Wake up, folk!", "Pulse of " + lsGet("RTPulse") + " is bad! Time to get on track!", "RTNotify");
				stimuli("shock", defInt, defAT, "Incoming Zap. Time to get on track!");
			}
			else if (prod < parseInt(lsGet("RTWarnLimit"))) {
				notifyUser("Come on, you can do better!", "Pulse of " + lsGet("RTPulse") + " ain't bad, but you are better than that!", "RTNotify");
				stimuli("beep", defInt, defAT, "Incoming Beep. Come on, you can do better!!!");
			}
			else if (prod > parseInt(lsGet("RTPosLimit"))) {
				notifyUser("Whoohoo!!! On fire!", "Pulse of " + lsGet("RTPulse") + " is damn solid! Rock on!", "RTNotify");
				stimuli("vibration", defInt, defAT, "You rock! Let Pavlok massage your wrist a bit!");
			}
			else if (prod < parseInt(lsGet("RTPosLimit"))) {
				notifyUser("Way to go!", "Pulse of " + lsGet("RTPulse") + " a good start! Keep improving!", "RTNotify");
			}
...
function notifyTabCount(tabs, situation) {
	...
	if (situation.status == "over") {
		...
		if (situation.trend != "lowering") {
			notMessage = "You opened more than " + lsGet("maxTabs") + " tabs! Close them down and keep your sanity!";
		}
...
function checkActiveDayHour() {
	var now = new Date();
	var start = lsGet("generalActiveTimeStart");
	var end = lsGet("generalActiveTimeEnd");
	// log("Now is: " + now + "\nStarts at: " + start + "\nEnds at: " + end);
...
function checkActiveDay(date) {
	...
	var activeDaysList = [];
	if (lsGet("sundayActive") == 'true') { activeDaysList.push(0); }
	if (lsGet("mondayActive") == 'true') { activeDaysList.push(1); }
	if (lsGet("tuesdayActive") == 'true') { activeDaysList.push(2); }
	if (lsGet("wednesdayActive") == 'true') { activeDaysList.push(3); }
	if (lsGet("thursdayActive") == 'true') { activeDaysList.push(4); }
	if (lsGet("fridayActive") == 'true') { activeDaysList.push(5); }
	if (lsGet("saturdayActive") == 'true') { activeDaysList.push(6); }
...
function CreateTabListeners(token) {
	...
	chrome.tabs.onCreated.addListener(function (tab) {
		if (isActive() && lsGet("tabNumbersActive") == "true") {
			countTabs(lsGet("tabCountAll"), evaluateTabCount);
...
var blackList = {
	...
	set: function (data) {
		if (data.receiver == "white") {
			lsSet("whiteList", JSON.stringify(data.list));
		}
		else if (data.receiver == "black") {
			lsSet("blackList", JSON.stringify(data.list));
		}
		else if (data.receiver == "global") {
			lsSet("blackGlobal", JSON.strigify(data.global));
		}
	},
	get: function (list) {
		var cList;
	...
	resolver: function (curPAVTab, curPAVUrl, curPAVDomain, callback) {
		...
		if (_result == true) {		//blacklisted site, quota exceeded
			...
			var firstZap = lsGet("firstZap") == "true";
			var timeWindowZap = parseInt(lsGet("timeWindow"));
			// var timeWindowZap = 3;
			if (instaZap) {
				timeWindowZap = 8;
				if (firstZap == false) {
	...
	checkForMigration: function () {
		...
		try { cBL = JSON.parse(lsGet("blackList")); }
		catch (err) {
			cBL = lsGet("blackList");
			var keys = cBL.split(",");
